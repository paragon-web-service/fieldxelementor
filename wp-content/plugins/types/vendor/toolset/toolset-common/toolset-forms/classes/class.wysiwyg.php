...
class WPToolset_Field_Wysiwyg extends WPToolset_Field_Textarea {
    ...
    protected function _editor(&$attributes) {
        ...
        wp_editor( $this->getValue(), $id, array(
			// We actually *do* use wpautop, but since the TinyMCE editor will be reinitialized in JS (search for
			// initializeWysiwygFields), doing this here could cause a double wpautop application in case that
			// race condition issue does NOT occur. And that would result in line breaks being lost.
            'wpautop' => false,
