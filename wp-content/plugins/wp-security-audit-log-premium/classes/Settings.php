...
class WSAL_Settings {
	...
	public static function get_working_dir_path_static( $path = '', $skip_creation = false, $ignore_site = false ) {
		...
		if ( ! $ignore_site && WpSecurityAuditLog::is_multisite() ) {
	...
	public function set_database_logging_disabled( $is_disabled ) {
		if ( $is_disabled ) {
			...
			$this->plugin->alerts->trigger_event( 6327, array( 'EventType' => 'disabled' ) );
		} else {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'db_logging_disabled' );
			$this->plugin->alerts->trigger_event( 6327, array( 'EventType' => 'enabled' ) );
		}
	}
	/* @premium:end */
	...
	public function set_report_pruning_enabled( $enabled ) {
		$boolean_value = \WSAL\Helpers\Options::string_to_bool( $enabled );
