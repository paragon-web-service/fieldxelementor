...
class WSAL_ViewManager {
	...
	public function __construct( WpSecurityAuditLog $plugin ) {
		...
		if ( wsal_freemius()->is_plan_or_trial__premium_only( 'starter' ) ) {
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/EmailNotifications.php';
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/Search.php';
		}

		if ( wsal_freemius()->is_plan_or_trial__premium_only( 'professional' ) ) {
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/EmailNotifications.php';
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/Search.php';
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/ExternalDB.php';
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/LogInUsers.php';
			$skip_views[] = WSAL_BASE_DIR . 'classes/Views/Reports.php';
		}
		/* @premium:end */
	...
	public function live_notifications__premium_only( $admin_bar ) {
		if ( $this->plugin->settings()->current_user_can( 'view' ) && is_admin() ) {
			...
			$event       = $this->plugin->alerts->get_admin_bar_event( 'page-refresh' === $adn_updates );
	...
	public function wsal_adminbar_events_refresh__premium_only() {
		...
		if ( $events_count ) {
			...
			if ( $events_count !== $new_count ) {
				$event = $this->plugin->alerts->get_admin_bar_event( true );
				$code  = $this->plugin->alerts->get_alert( $event['alert_id'] );
			...
			echo wp_json_encode(
				array(
					'success' => false,
					'message' => esc_html__( 'Log count parameter expected.', 'wp-security-audit-log' ),
				)
			);
		}
		die();
