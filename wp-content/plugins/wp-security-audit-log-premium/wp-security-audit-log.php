...
if ( ! function_exists( 'wsal_freemius' ) ) {
	...
	if ( ! class_exists( 'WpSecurityAuditLog' ) ) {
		...
		class WpSecurityAuditLog {
					...
					if ( Settings_Helper::is_archiving_enabled() ) {
						...
						if ( ( ! isset( $get_page ) || 'wsal-auditlog' !== $get_page ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
							...
							if ( $selected_db && ( get_current_user_id() === $selected_db_user ) ) {
								...
								self::delete_transient( 'wsal_wp_selected_db' );
								self::delete_transient( 'wsal_wp_selected_db_user' );
							}
						}
					}
					/* @premium:end */
			 ...
			 * @return array|null
			 */
			public function get_notifications_setting( $opt_prefix ) {
				$this->include_options_helper();
				return $this->options_helper->get_notifications_setting( WSAL_PREFIX . $opt_prefix );
			}
			...
			public function get_notification( $id ) {
				$this->include_options_helper();
				return $this->options_helper->get_notification( $id );
			}
			 ...
			 * @return int
			 */
			public function count_notifications( $opt_prefix ) {
				$this->include_options_helper();
				return $this->options_helper->count_notifications( $opt_prefix );
			}
			/* @premium:end */
			/**
		...
		$autoloader_file_path = plugin_dir_path( __FILE__ ) . implode(
			DIRECTORY_SEPARATOR,
			array(
				'vendor',
				'autoload.php',
